...
export function GroundBattleView({ state, battle, mode = 'battle', movementSmoothingMs = 0, onFocus, onManeuver, onHold, onLoad, onEvacuate, onExit }: {
  state: GameState;
  battle: GroundBattle;
  mode?: 'battle' | 'deployment';
  ...
  onLoad: (planetId: string, transportId: string, unitIds: string[]) => void;
  onEvacuate: (planetId: string, transportIds: string[]) => void;
  onExit: () => void;
}) {
  const planet = state.planets.find(p => p.id === battle.planetId)!;
  const deploymentMode = mode === 'deployment';
  ...
  const onEvacuateRef = useRef(onEvacuate);
  selectedUnitIdsRef.current = selectedUnitIds;
  onHoldRef.current = onHold;
  onEvacuateRef.current = onEvacuate;
  ...
  const allUnitsRef = useRef(allUnits);
  allUnitsRef.current = allUnits;
  const selectedUnits = deploymentMode ? [] : allUnits.filter(unit => unit.faction === 'player' && selectedUnitIds.includes(unit.id));
  const selectedMobileUnits = selectedUnits.filter(unit => !unit.landedTransport);
  const selectedTransports = selectedUnits.filter(unit => unit.landedTransport);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.code === 'KeyH') {
        ...
        const unitIds = allUnitsRef.current.filter(unit => selectedUnitIdsRef.current.includes(unit.id) && unit.faction === 'player' && !unit.landedTransport).map(unit => unit.id);
      ...
      if (event.code === 'KeyE') {
        if (deploymentMode) return;
        const transportIds = allUnitsRef.current.filter(unit => selectedUnitIdsRef.current.includes(unit.id) && unit.faction === 'player' && unit.landedTransport && (unit.cargo?.length ?? 0) > 0).map(unit => unit.id);
        if (!transportIds.length) return;
        event.preventDefault();
        if (!event.repeat) onEvacuateRef.current(battle.planetId, transportIds);
        return;
      }
  ...
  }, [battle.planetId, deploymentMode]);
  useEffect(() => {
  ...
  const visibleToPlayer = (unit: Unit) => deploymentMode || unit.faction === 'player' || friendlyUnits.some(observer =>
    Math.hypot((unit.battleX ?? 0) - (observer.battleX ?? 0), (unit.battleY ?? 0) - (observer.battleY ?? 0))
      <= groundUnitVisionRange(observer));
  ...
  const combatant = (unit: Unit, index: number) => {
    ...
    const selectable = !deploymentMode && friendly && !unit.sourceBuildingId;
  ...
  >
    ...
    <div className="battle-hud"><small>{deploymentMode ? 'GROUND DEPLOYMENT' : 'GROUND ENGAGEMENT'} // {planet.name.toUpperCase()}</small>{deploymentMode
      ? <><b>{allUnits.length - activeDefenses} GROUND UNIT{allUnits.length - activeDefenses === 1 ? '' : 'S'} · {activeDefenses} DEFENSE{activeDefenses === 1 ? '' : 'S'}</b><p>Read-only surface overview · inspect the stationed army, fortifications, terrain, and weapon ranges.</p></>
      : <><b>{friendlyUnits.length} FRIENDLY <span>VS</span> {visibleHostiles.length} CONTACT{visibleHostiles.length === 1 ? '' : 'S'}</b><p>Right-click to attack-move · double right-click to force movement · right-click a landed transport to load · E evacuates loaded selected transports. Rocks block movement for ground units · Forests grant 30% cover · Each unit reveals its own sight radius.</p>{playerOrbitalSupport && <em>{playerShips.length} SHIP{playerShips.length === 1 ? '' : 'S'} PROVIDING ORBITAL FIRE · {playerShips.length} DAMAGE/S</em>}</>}{activeDefenses > 0 && <em>{activeDefenses} FORTIFIED DEFENSE{activeDefenses === 1 ? '' : 'S'} ONLINE</em>}</div>
    <div className="battle-scroll" ref={scrollRef} aria-label="Scrollable ground battlefield">
      ...
      <div className={`battle-canvas ${selectedMobileUnits.length ? 'commanding-ground-units' : ''}`} style={{ '--battlefield-width': `${GROUND_BATTLEFIELD_WIDTH}px`, '--battlefield-height': `${GROUND_BATTLEFIELD_HEIGHT}px`, transform: `scale(${battleZoom})` } as React.CSSProperties} onPointerDown={event => {
        if (deploymentMode || event.button !== 0) return;
        ...
        event.preventDefault();
        if (deploymentMode || !selectedMobileUnits.length) return;
        const rect = event.currentTarget.getBoundingClientRect();
        ...
        const unitIds = selectedMobileUnits.map(unit => unit.id);
        ...
        {!deploymentMode && <div className="front-line"><i /><span>CONTESTED ZONE</span><i /></div>}
        ...
        {!deploymentMode && <svg className="ground-fog" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
          <defs><mask id={fogMaskId} maskUnits="userSpaceOnUse"><rect width="100" height="100" fill="white" />{friendlyUnits.map(unit => <circle key={`sight-${unit.id}`} cx={unit.battleX ?? 0} cy={unit.battleY ?? 0} r={groundUnitVisionRange(unit)} fill="black" />)}</mask></defs>
    ...
    {!deploymentMode && <div className="battle-selection-status">{selectedUnits.length ? `${selectedUnits.length} UNIT${selectedUnits.length === 1 ? '' : 'S'} SELECTED${selectedTransports.length ? ' · E EVACUATE LOADED TRANSPORTS' : ' · H HOLD · DOUBLE RIGHT-CLICK FORCE MOVE'}` : 'DRAG-SELECT FRIENDLY TROOPS OR TRANSPORTS TO ISSUE ORDERS'}</div>}
    ...
    <div className="battle-help">← EXIT {deploymentMode ? 'SURFACE MAP' : 'BATTLEFIELD'} <span>{deploymentMode ? 'Deployment overview is visual only' : 'Units retaliate when attacked · hold-position units fire without chasing'}</span></div>
  </div>;
}
