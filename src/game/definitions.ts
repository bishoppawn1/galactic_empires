import type { Building, BuildingKind, DefenseBuildingKind, Definition, GroundUnitKind, PlayableFaction, ResearchId, ResourcePool, SpaceUnitKind, TitanUpgradeId, Unit, UnitDefinition, UnitKind } from './types';
...
export const UNITS: Record<UnitKind, UnitDefinition> = {
  ...
  dreadnought: { label: 'Titan Dreadnought', description: 'Ultimate capital ship whose independent siege cores engage multiple warships.', cost: pool(900, 680, 460), time: 120, factory: 'space', hp: 1900, shields: 1050, range: 460, moveSpeed: 0, weapon: { label: 'Tri-Core Siege Beams', damage: 10, cooldown: 1, projectiles: 3, effect: 'siege' }, requires: 'titanEngineering', advancedFactory: true, ability: { kind: 'triCoreBarrage', label: 'Tri-Core Barrage', description: 'Each salvo also strikes up to two additional hostile ships for 50% damage.' } },
  ...
  worldEater: { label: 'World Eater', description: 'The apex organism, able to feed on orbital fortresses in a few passes.', cost: pool(860, 630, 410), time: 112, factory: 'space', hp: 2200, shields: 650, range: 480, moveSpeed: 0, weapon: { label: 'Devouring Beam', damage: 12, cooldown: 1.1, projectiles: 3, effect: 'siege' }, requires: 'titanEngineering', advancedFactory: true, ability: { kind: 'planetCracker', label: 'Planet Cracker', description: 'Deals double damage to orbital defense platforms and restores hull equal to 10% of that damage.' } },
  ...AEGIS_UNITS,
  ...COVENANT_UNITS,
};
...
const BROOD_RESEARCH_UNLOCKS: Partial<Record<ResearchId, string[]>> = {
  ...
  industrialIteration: ['Repeatable · +5% gestation speed per level'], resourceSynthesis: ['Repeatable · +5% planetary biomass per level'],
...
export const GRAVITY_WELL_RADIUS = 960;
export const MAX_SHIP_ORBIT_RADIUS = GRAVITY_WELL_RADIUS - 80;
export const MIN_SHIP_ORBIT_SEPARATION = 76;
...
export const unitWeaponDamage = (unit: Unit) => UNITS[unit.kind].weapon.damage * (unit.titanUpgrades?.includes('siegeCore') ? 1.35 : 1);
