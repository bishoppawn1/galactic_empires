import { RESEARCH, empireCivilization, formatFactionCost, researchLabCount, researchSpeedMultiplier, researchUnlocksForCivilization, type GameCommand, type GameState, type ResearchId } from '../../game';

function ResearchNode({ id, state, hasLab, speed, act }: { id: ResearchId; state: GameState; hasLab: boolean; speed: number; act: (command: GameCommand) => void }) {
  ...
  const buttonLabel = done ? 'COMPLETE' : active ? (speed ? `${Math.ceil(active.remaining / speed)}s` : 'OFFLINE') : !hasLab ? 'LAB REQUIRED' : !prerequisiteMet ? 'PREREQUISITE' : 'RESEARCH';
  ...
  return <article className={`tech-node ${status}`} data-tech-id={id} data-requires={def.requires ?? ''}>
    ...
    {active ? <div className="research-progress" aria-label={`${def.label} progress`}><i style={{ width: `${100 * (1 - active.remaining / active.total)}%` }} /></div> : <em>{formatFactionCost(def.cost, empireCivilization(state))} · {speed ? Math.ceil(def.time! / speed) : def.time}s</em>}
...
export function ResearchView({ state, act }: { state: GameState; act: (command: GameCommand) => void }) {
  ...
  return <main className="research-view" aria-label="Research tech tree">
    <header className="research-hero"><div><small>SCIENCE DIRECTORATE // EMPIRE-WIDE</small><h1>Technology tree</h1><p>Develop foundational industry, then choose the doctrines that shape your expansion.</p></div>
      <div className="research-stats"><span><b>{state.completedResearch.length}</b> COMPLETE</span><span><b>{state.researchQueue.length}</b> ACTIVE</span><span><b>{labs}</b> LABS</span><span><b>{speed.toFixed(1)}×</b> SPEED</span></div>
    </header>
    ...
    <section className="tech-tree expanded-tech-tree" aria-label="Technology prerequisites">
      {tiers.map((ids, tier) => <div className="tech-tier" data-tier={tier + 1} key={tier}>
        ...
        <div>{ids.map(id => <ResearchNode key={id} id={id} state={state} hasLab={hasLab} speed={speed} act={act} />)}</div>
      </div>)}
    </section>
  </main>;
}
