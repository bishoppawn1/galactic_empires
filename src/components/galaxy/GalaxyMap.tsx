...
import {
  ...
  MAX_LARGE_FLEET_VISUAL_SHOTS, evenlySampleVisuals, usesLargeFleetRenderBudget,
} from './renderBudget';
...
function ShipMapStatusBars({ ship, visible }: { ship: Unit; visible: boolean }) {
  ...
  return <span className="ship-map-status camera-billboard" aria-hidden="true">
...
export function GalaxyMap({ state, selectedId, selectedShipIds, selectedYardIds, zoom: controlledZoom, movementSmoothingMs = 0, onZoomChange, onSelect, onOrderToPlanet, onSelectShip, onSelectSpaceYard, onGroupSelect, onManeuver, onTargetDefense, onUpgradeTitan }: {
  ...
  const ownershipCounts = {
    ...
    ancientTemple: state.planets.filter(planet => systemKind(planet) === 'ancientTemple').length,
  };
  ...
  const strategicShipMarkers = usesStrategicShipMarkers(zoom);
  ...
  return <main className={`galaxy ${camera3D ? 'view-3d' : 'view-2d'} ${movementSmoothingMs ? 'network-smoothed' : ''} ${strategicShipMarkers ? 'strategic-ship-markers' : ''} ${largeFleetRendering ? 'large-fleet-rendering' : ''} ${selectedOrigin ? 'issuing-order' : ''} ${selectedYardIds.length ? 'selecting-yards' : ''}`} aria-label="Galaxy map" data-large-fleet-rendering={largeFleetRendering} data-strategic-ship-markers={strategicShipMarkers} data-movement-smoothing-ms={movementSmoothingMs} style={{ '--movement-smoothing': `${movementSmoothingMs || 110}ms` } as React.CSSProperties}>
    <div className="galaxy-scroll" ref={scrollRef}>
      <div className="galaxy-canvas" style={{
        ...
        zoom,
        '--strategic-marker-size': `${STRATEGIC_SHIP_MARKER_SCREEN_SIZE / zoom}px`,
        '--strategic-marker-line': `${1.5 / zoom}px`,
        ...(camera3D ? {
          transform: `rotateZ(${camera.yaw}deg) rotateX(${camera.pitch}deg)`,
          transformOrigin: '50% 50%',
          '--camera-counter-scale': 1 / cameraDepth(camera.pitch),
          '--camera-inverse-pitch': `${-camera.pitch}deg`,
          '--camera-inverse-yaw': `${-camera.yaw}deg`,
        ...
        const hit = inspectableShipAtPoint(state, point.x, point.y, zoom);
        if (!hit) return;
        ...
        {state.planets.map(p => {
          ...
          return <button key={p.id} aria-label={`${p.name} ${kind === 'planet' ? ownerLabel(p.owner) : kindLabel}`} className={`planet-node system-${kind} ${selectedId === p.id ? 'selected' : ''} ${p.owner ?? 'neutral'}`} style={{ left: `${p.x}%`, top: `${p.y}%`, '--planet': planetDisplayColor(p), '--gravity-well-size': `${GRAVITY_WELL_RADIUS * 2}px`, '--gravity-well-offset': `${PLANET_HIT_SIZE / 2 - GRAVITY_WELL_RADIUS}px` } as React.CSSProperties} onClick={event => { event.stopPropagation(); onSelect(p.id); }} onContextMenu={event => { event.preventDefault(); event.stopPropagation(); onOrderToPlanet(p.id); }}>
            {(battle || hostileOrbit) && <span className="battle-pulse camera-billboard">⚔</span>}<span className="orbit-zone" /><span className="ownership-ring" /><span className="orbit-ring" />{world ? <span className={`planet-sphere ${kind === 'pirateBase' ? 'pirate-world' : ''}`} /> : <span className={`system-object ${kind}`} aria-hidden="true"><i /></span>}
            <span className="faction-badge camera-billboard">{kind === 'planet' || kind === 'ancientTemple' || (kind === 'pirateBase' && p.owner) ? planetFactionBadge(p.owner) : kind === 'pirateBase' ? 'PIRATES' : 'ANOMALY'}</span><span className="planet-name camera-billboard">{p.name}</span><span className="planet-status camera-billboard">{kindLabel}</span>{kind === 'nebula' && p.orbitUnits.length !== visibleShips.length ? <span className="orbit-count jammed camera-billboard">SIGNALS JAMMED</span> : !!visibleShips.length && <span className="orbit-count camera-billboard">◈ {visibleShips.length}</span>}
          </button>;
        })}
        {state.planets.flatMap(p => {
          ...
          return yards.map((yard, index) => {
            ...
            const content = <><span className="camera-billboard">{tier === 1 ? 'Y' : tier === 2 ? 'A' : 'X'}{index + 1}</span><small className="camera-billboard">{p.owner !== 'player' ? `HOSTILE ${shortLabel}` : shortLabel}</small></>;
        ...
        {state.planets.flatMap(p => visibleOrbitUnits(p).flatMap((ship, index) => {
          ...
          const damageMultiplier = titanWeaponDamageMultiplier(ship);
          const rangeMultiplier = titanWeaponRangeMultiplier(ship);
          const systems = weapons.map(weapon => `${weapon.mounts}× ${weapon.label} · ${Number((weapon.damage * damageMultiplier).toFixed(2))} damage each · ${weapon.cooldown}s reload · range ${Math.round(weapon.range * rangeMultiplier)}`).join(' + ');
          ...
          const phaseStacks = phaseControlStackCount(p.orbitUnits, ship);
          return <button key={ship.id} aria-label={`${UNITS[ship.kind].label}${approach} ${p.name}${phaseStacks ? ', phase suppressed' : ''}`} title={`Armor ${Math.round(shipArmor(ship.kind as SpaceUnitKind) * 100)}% · ${systems}${ability ? ` · ${ability.label}: ${ability.description}` : ''}`} className={`orbit-ship ${ship.faction} ${ship.phaseArrival ? 'phase-arrival' : ''} ${ship.pendingLanding ? 'landing-approach' : ''} ${ship.pendingEmbark ? 'embark-approach' : ''} ${ship.docked ? 'docked' : ''} ${phaseStacks ? 'phase-slowed' : ''} ${selected ? 'selected' : ''}`} style={{ left: position.x, top: position.y, '--ship-heading': `${orbitShipHeading(ship)}deg`, '--ship-display-size': `${displaySize}px`, '--ship-label-offset': `${displaySize / 2 + 8}px`, '--ship-status-offset': `${displaySize / 2 + 7}px`, '--ship-status-width': `${Math.min(68, Math.max(48, displaySize * .55))}px` } as React.CSSProperties} onClick={event => { event.stopPropagation(); onSelectShip(p.id, ship.id, selectable && event.shiftKey); }}><i className="ship-range-ring" style={{ '--ship-range': `${unitMaximumWeaponRange(ship) * 2}px` } as React.CSSProperties} />{ability?.kind === 'shieldProjection' && <i className="ship-ability-ring" aria-label="Shield Projection radius" style={{ '--ship-ability-range': `${AEGIS_SHIELD_PROJECTION_RANGE * 2}px` } as React.CSSProperties} />}{ability?.kind === 'phaseControl' && <i className="ship-ability-ring phase-control-ring" aria-label={`${ability.label} radius`} style={{ '--ship-ability-range': `${UNITS[ship.kind].range * 2}px` } as React.CSSProperties} />}{repairRange > 0 && <i className="ship-ability-ring covenant-repair" aria-label={`${ability!.label} radius`} style={{ '--ship-ability-range': `${repairRange * 2}px` } as React.CSSProperties} />}{selectable && <i className="ship-control-frame" aria-hidden="true" />}{strategicShipMarkers ? <StrategicShipMarker kind={ship.kind as SpaceUnitKind} /> : <ShipImage kind={ship.kind} volumetric={camera3D && !largeFleetRendering} />}<ShipMapStatusBars ship={ship} visible={selected} />{phaseStacks > 0 && <small className="phase-control-status camera-billboard">PHASE SUPPRESSED</small>}{capacity && <small className={`transport-capacity camera-billboard ${cargoCount >= capacity ? 'full' : ''}`} aria-label={`Cargo ${cargoCount} of ${capacity}`}>{ship.pendingLanding ? 'LANDING · ' : ship.pendingEmbark ? 'EMBARKING · ' : ship.docked ? 'DOCKED · ' : ''}{cargoCount}/{capacity}</small>}</button>;
        }))}
        {state.fleets.flatMap(fleet => {
          ...
          const phaseStacks = position.phase === 'tunnel' ? 0 : fleetPhaseControlStackCount(state, fleet);
          const gateBlocked = phaseGateBlocked(state, fleet);
          const className = `transit-ship ${fleet.faction} ${position.phase} ${selectable ? 'interruptible' : 'committed'} ${phaseStacks ? 'phase-slowed' : ''} ${gateBlocked ? 'gate-blocked' : ''} ${selectedShipIds.includes(fleet.unit.id) ? 'selected' : ''}`;
          ...
          const content = <>{strategicShipMarkers ? <StrategicShipMarker kind={fleet.unit.kind as SpaceUnitKind} /> : <ShipImage kind={fleet.unit.kind} volumetric={camera3D && !largeFleetRendering} />}{selectable && <i className="ship-control-frame" aria-hidden="true" />}<ShipMapStatusBars ship={fleet.unit} visible={selectedShipIds.includes(fleet.unit.id)} />{gateBlocked ? <small className="phase-control-status camera-billboard">PHASE LOCKED</small> : phaseStacks > 0 ? <small className="phase-control-status camera-billboard">PHASE SUPPRESSED</small> : null}</>;
