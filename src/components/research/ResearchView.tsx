import {
  ...
  researchCost, researchDefinitionForCivilization, researchLabCount, researchLevel, researchSpeedMultiplier, researchTime,
  researchUnlocksForCivilization, type GameCommand, type GameState, type PlayableFaction, type ResearchId,
} from '../../game';
...
function ResearchNode({ id, state, hasLab, speed, civilization, act }: {
  id: ResearchId; state: GameState; hasLab: boolean; speed: number; civilization: PlayableFaction; act: (command: GameCommand) => void;
}) {
...
export function ResearchView({ state, act }: { state: GameState; act: (command: GameCommand) => void }) {
  ...
  return <main className={`research-view research-${civilization}`} aria-label="Research tech tree">
    <header className="research-hero"><div><small>{faction.label.toUpperCase()} // STRATEGIC RESEARCH MATRIX</small><h1>{faction.label} technology lattice</h1><p>{FACTION_RESEARCH_INTRO[civilization]}</p></div>
      <div className="research-stats"><span><b>{discoveries}</b> DISCOVERIES</span><span><b>{iterations}</b> ITERATIONS</span><span><b>{state.researchQueue.length}</b> ACTIVE</span><span><b>{labs}</b> LABS</span><span><b>{speed.toFixed(1)}×</b> SPEED</span></div>
    </header>
    ...
    <section className="tech-tree expanded-tech-tree research-graph" aria-label={`${faction.label} technology prerequisites`}>
      <div className="research-graph-canvas" style={{ width: GRAPH_WIDTH, height: GRAPH_HEIGHT + NODE_HEIGHT }}>
        ...
        {researchIds.map(id => <ResearchNode key={id} id={id} state={state} hasLab={hasLab} speed={speed} civilization={civilization} act={act} />)}
      </div>
    </section>
  </main>;
}
