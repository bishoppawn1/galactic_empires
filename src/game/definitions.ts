...
export const UNITS: Record<UnitKind, UnitDefinition> = {
  ...
  recon: { label: 'Light Recon Vehicle', description: 'Fast surface scout with twin autocannons.', cost: pool(55, 18, 9), time: 16, factory: 'ground', hp: 130, shields: 15, range: 11, moveSpeed: 11, groundTargeting: 'surface', weapon: { label: 'Twin Autocannon', damage: 1, cooldown: .5, projectiles: 2, effect: 'kinetic' } },
  lightTank: { label: 'Light Tank', description: 'Durable surface armor carrying a medium accelerator cannon.', cost: pool(82, 30, 15), time: 22, factory: 'ground', hp: 220, shields: 30, range: 16, moveSpeed: 5, groundTargeting: 'surface', weapon: { label: 'Accelerator Cannon', damage: 7, cooldown: 1.2, projectiles: 1, effect: 'kinetic' } },
  ...
  flakRover: { label: 'Skyguard Flak Rover', description: 'A mobile tracking battery that tears into aircraft and exposed infantry.', cost: pool(70, 28, 14), time: 20, factory: 'ground', hp: 145, shields: 20, range: 18, moveSpeed: 8, weapon: { label: 'Twin Tracking Autocannons', damage: .8, cooldown: .45, projectiles: 3, effect: 'kinetic' }, ability: { kind: 'groundFlak', label: 'Tracking Flak', description: 'Deals 50% more damage to infantry and flying ground units.' } },
  dragonflyScout: { label: 'Dragonfly Scout', description: 'A fast VTOL scout with a broad sensor footprint and light autocannons.', cost: pool(62, 28, 12), time: 18, factory: 'ground', hp: 105, shields: 25, range: 13, visionRange: 40, moveSpeed: 14, flying: true, weapon: { label: 'Twin Scout Autocannons', damage: .7, cooldown: .45, projectiles: 2, effect: 'kinetic' }, ability: { kind: 'aerialRecon', label: 'Aerial Recon', description: 'Flies over rocks and reveals a much wider area than line units.' } },
  falconGunship: { label: 'Falcon Gunship', description: 'An atmospheric bomber that crosses terrain to deliver sustained surface fire.', cost: pool(155, 92, 46), time: 38, factory: 'ground', hp: 260, shields: 65, range: 24, visionRange: 30, moveSpeed: 7.5, flying: true, groundTargeting: 'surface', weapon: { label: 'Paired Falcon Cannons', damage: 2.8, cooldown: .75, projectiles: 2, effect: 'pulse' }, requires: 'groundWarfare', advancedFactory: true, ability: { kind: 'airSupport', label: 'Air Support', description: 'Flies over rocks, ignores forest cover, and attacks surface targets only.' } },
  ...
  reconCutter: { label: 'Recon Cutter', description: 'A lightly armed Tier 1 scout whose oversized phase drive crosses systems at extreme speed.', cost: pool(95, 58, 26), time: 10, factory: 'space', spaceTier: 1, hp: 150, shields: 75, range: 200, moveSpeed: 0, orbitSpeedMultiplier: 2.5, weapon: { label: 'Survey Laser', damage: .18, cooldown: .8, projectiles: 1, effect: 'laser' }, ability: { kind: 'reconDrive', label: 'High-Velocity Recon Drive', description: 'Moves through gravity wells and phase lanes at 2.5× normal ship speed, but carries only a light defensive weapon.' } },
  ...
  railgunTank: { label: 'Railgun Tank', description: 'Fast heavy surface armor with a deliberate hypervelocity rail shot.', cost: pool(175, 110, 58), time: 42, factory: 'ground', hp: 430, shields: 75, range: 25, moveSpeed: 5.2, groundTargeting: 'surface', weapon: { label: 'Hypervelocity Railgun', damage: 24, cooldown: 1.9, projectiles: 1, effect: 'railgun' }, requires: 'heavyArmor', advancedFactory: true },
  ...
  plasmaTank: { label: 'Plasma Tank', description: 'Heavy shielded surface armor built around a plasma lance.', cost: pool(155, 92, 48), time: 38, factory: 'ground', hp: 390, shields: 90, range: 19, moveSpeed: 4.2, groundTargeting: 'surface', weapon: { label: 'Plasma Lance', damage: 13, cooldown: 1.25, projectiles: 1, effect: 'plasma' }, requires: 'heavyArmor', advancedFactory: true },
  siegeWalker: { label: 'Siege Walker', description: 'Long-range armored surface platform with a fortress-breaking cannon.', cost: pool(210, 135, 72), time: 52, factory: 'ground', hp: 520, shields: 120, range: 36, moveSpeed: 2.6, groundTargeting: 'surface', weapon: { label: 'Quake Siege Cannon', damage: 31, cooldown: 2.1, projectiles: 1, effect: 'siege' }, requires: 'heavyArmor', advancedFactory: true },
  ...
  spineFlak: { label: 'Spine Flak Beast', description: 'A mobile interception organism that fills the sky and exposed infantry lines with hunting spines.', cost: pool(60, 20, 8), time: 17, factory: 'ground', hp: 135, shields: 0, range: 19, moveSpeed: 8.5, weapon: { label: 'Hunter Spine Fan', damage: .75, cooldown: .55, projectiles: 4, effect: 'kinetic' }, ability: { kind: 'groundFlak', label: 'Hunter Spines', description: 'Deals 50% more damage to infantry and flying ground units.' } },
  razorwing: { label: 'Razorwing', description: 'A darting aerial hunter whose sensory crown tastes movement across the battlefield.', cost: pool(54, 20, 8), time: 15, factory: 'ground', hp: 90, shields: 0, range: 10, visionRange: 41, moveSpeed: 15, flying: true, weapon: { label: 'Razor Spine Volley', damage: .65, cooldown: .35, projectiles: 3, effect: 'kinetic' }, ability: { kind: 'aerialRecon', label: 'Sky Senses', description: 'Flies over rocks and reveals a much wider area than line organisms.' } },
  sporewing: { label: 'Sporewing', description: 'A winged bomber that drops volatile cysts onto surface formations.', cost: pool(142, 76, 34), time: 35, factory: 'ground', hp: 280, shields: 0, range: 27, visionRange: 33, moveSpeed: 6.5, flying: true, groundTargeting: 'surface', weapon: { label: 'Volatile Spore Cyst', damage: 12, cooldown: 2, projectiles: 1, effect: 'artillery' }, requires: 'groundWarfare', advancedFactory: true, ability: { kind: 'burstSpores', label: 'Sporefall', description: 'Deals 35% splash damage to nearby surface enemies.' } },
  ...
  crusherBeast: { label: 'Crusher Beast', description: 'A massive surface shock organism bred to tear fortified positions apart.', cost: pool(160, 92, 42), time: 38, factory: 'ground', hp: 500, shields: 0, range: 10, moveSpeed: 6.5, groundTargeting: 'surface', weapon: { label: 'Siege Horn', damage: 30, cooldown: 1.6, projectiles: 1, effect: 'kinetic' }, requires: 'heavyArmor', advancedFactory: true, ability: { kind: 'siegeCharge', label: 'Siege Charge', description: 'Deals double damage to Ground Defense emplacements.' } },
  acidBehemoth: { label: 'Acid Behemoth', description: 'A towering surface reservoir beast that leaves whole formations vulnerable.', cost: pool(148, 84, 40), time: 36, factory: 'ground', hp: 410, shields: 0, range: 23, moveSpeed: 4, groundTargeting: 'surface', weapon: { label: 'Bile Lance', damage: 15, cooldown: 1.15, projectiles: 1, effect: 'plasma' }, requires: 'heavyArmor', advancedFactory: true, ability: { kind: 'corrosiveBile', label: 'Corrosive Torrent', description: 'Hits corrode targets for 5 seconds, increasing all damage they take by 35%.' } },
  siegeCrawler: { label: 'Siege Crawler', description: 'A living surface fortress whose quake cysts engulf clustered defenders.', cost: pool(200, 124, 64), time: 48, factory: 'ground', hp: 590, shields: 0, range: 40, moveSpeed: 2.4, groundTargeting: 'surface', weapon: { label: 'Quake Cyst', damage: 29, cooldown: 2, projectiles: 1, effect: 'siege' }, requires: 'heavyArmor', advancedFactory: true, ability: { kind: 'burstSpores', label: 'Quake Bloom', description: 'Deals 35% splash damage to nearby surface enemies.' } },
  ...
  broodSeeker: { label: 'Seeker', description: 'A fragile Tier 1 reconnaissance organism bred around an oversized migration organ.', cost: pool(85, 50, 22), time: 10, factory: 'space', spaceTier: 1, hp: 175, shields: 35, range: 190, moveSpeed: 0, orbitSpeedMultiplier: 2.5, weapon: { label: 'Sighting Spore', damage: .16, cooldown: .75, projectiles: 2, effect: 'drone' }, ability: { kind: 'reconDrive', label: 'Migratory Surge', description: 'Moves through gravity wells and phase lanes at 2.5× normal ship speed, but carries only light sighting spores.' } },
...
export const SHIP_TURN_COAST_SPEED_MULTIPLIER = .35;
export const LANDING_APPROACH_SPEED = 14;
export const SYSTEM_EXIT_SPEED = 18;
export const PHASE_GATE_CHARGE_SECONDS = 2;

export const COALITION_GROUND_KINDS: GroundUnitKind[] = ['infantry', 'antiVehicle', 'recon', 'lightTank', 'artillery', 'flakRover', 'dragonflyScout', 'falconGunship', 'shockTrooper', 'railgunTank', 'plasmaTank', 'siegeWalker'];
export const BROOD_GROUND_KINDS: GroundUnitKind[] = ['broodling', 'acidSpitter', 'skitterer', 'carapaceBeast', 'sporeLobber', 'spineFlak', 'razorwing', 'sporewing', 'synapseGuard', 'crusherBeast', 'acidBehemoth', 'siegeCrawler'];
...
export const GROUND_UNIT_VISION_RANGES: Record<GroundUnitKind, number> = {
  infantry: 22, antiVehicle: 24, recon: 36, lightTank: 20, artillery: 24, flakRover: 29, dragonflyScout: 40, falconGunship: 30,
  shockTrooper: 24, railgunTank: 27, plasmaTank: 23, siegeWalker: 28, defenseTurret: 34,
  broodling: 19, acidSpitter: 25, skitterer: 38, carapaceBeast: 18, sporeLobber: 25, spineFlak: 30, razorwing: 41, sporewing: 33,
  synapseGuard: 28, crusherBeast: 19, acidBehemoth: 26, siegeCrawler: 30, spineTower: 36,
  aegisWarden: 25, aegisBastionTank: 22, aegisRampartArtillery: 28, aegisSkyguard: 32, aegisSeraphSkimmer: 42, aegisHaloGunship: 32,
  ...
  covenantFlakEngine: 31, covenantWaspDrone: 40, covenantFurnaceGunship: 30, covenantJuggernaut: 30, covenantBulwark: 36,
};
...
export const INFANTRY_GROUND_KINDS: ReadonlySet<GroundUnitKind> = new Set([
  'infantry', 'antiVehicle', 'shockTrooper',
  'broodling', 'acidSpitter', 'synapseGuard',
  'aegisWarden', 'aegisPaladinGuard',
  'covenantCohort',
]);
export const isInfantryGroundUnit = (unit: Unit) => INFANTRY_GROUND_KINDS.has(unit.kind as GroundUnitKind);
export const canGroundUnitAttackTarget = (attacker: Unit, target: Unit) =>
  UNITS[attacker.kind].groundTargeting !== 'surface' || !isFlyingGroundUnit(target);
...
const BROOD_EQUIVALENTS: Partial<Record<UnitKind, UnitKind>> = {
  ...
  flakRover: 'spineFlak', dragonflyScout: 'razorwing', falconGunship: 'sporewing',
...
const AEGIS_EQUIVALENTS: Partial<Record<UnitKind, UnitKind>> = {
  ...
  flakRover: 'aegisSkyguard', dragonflyScout: 'aegisSeraphSkimmer', falconGunship: 'aegisHaloGunship',
...
const COVENANT_EQUIVALENTS: Partial<Record<UnitKind, UnitKind>> = {
  ...
  flakRover: 'covenantFlakEngine', dragonflyScout: 'covenantWaspDrone', falconGunship: 'covenantFurnaceGunship',
