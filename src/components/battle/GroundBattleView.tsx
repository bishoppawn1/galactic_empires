...
export function GroundBattleView({ state, battle, mode = 'battle', movementSmoothingMs = 0, onFocus, onManeuver, onHold, onExit }: {
  state: GameState;
  battle: GroundBattle;
  ...
  const selectedUnits = deploymentMode ? [] : allUnits.filter(unit => unit.faction === 'player' && !unit.landedTransport && selectedUnitIds.includes(unit.id));
  ...
  const combatant = (unit: Unit, index: number) => {
    ...
    const selectable = !deploymentMode && friendly && !unit.sourceBuildingId && !unit.landedTransport;
  ...
  >
    ...
    <div className="battle-scroll" ref={scrollRef} aria-label="Scrollable ground battlefield">
      ...
      <div className={`battle-canvas ${selectedUnits.length ? 'commanding-ground-units' : ''}`} style={{ '--battlefield-width': `${GROUND_BATTLEFIELD_WIDTH}px`, '--battlefield-height': `${GROUND_BATTLEFIELD_HEIGHT}px`, transform: `scale(${battleZoom})` } as React.CSSProperties} onPointerDown={event => {
        ...
        event.preventDefault();
        if (deploymentMode || !selectedUnits.length) return;
        const rect = event.currentTarget.getBoundingClientRect();
    ...
    {!deploymentMode && <div className="battle-selection-status">{selectedUnits.length ? `${selectedUnits.length} UNIT${selectedUnits.length === 1 ? '' : 'S'} SELECTED · H HOLD · DOUBLE RIGHT-CLICK FORCE MOVE` : 'DRAG-SELECT FRIENDLY TROOPS TO ISSUE ORDERS'}</div>}
