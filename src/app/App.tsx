...
import {
  GUEST_STATE_TRANSITION_MS, hostMultiplayer, joinMultiplayer, matchSlotsForLobby,
...
export default function App() {
  ...
  const guestMovementSmoothingMs = controllerRef.current && !controllerRef.current.isHost
  ...
  if (battle) return <><ResourceBar state={presentedState} view="galaxy" onViewChange={changeView} act={issue} /><GroundBattleView state={presentedState} battle={battle} movementSmoothingMs={guestMovementSmoothingMs} onFocus={(planetId, targetId) => issue({ type: 'battleFocus', planetId, targetId })} onManeuver={(planetId, unitIds, battleX, battleY, forceMove) => issue({ type: 'battleManeuver', planetId, unitIds, battleX, battleY, forceMove })} onHold={(planetId, unitIds) => issue({ type: 'battleHold', planetId, unitIds })} onLoad={(planetId, transportId, unitIds) => issue({ type: 'battleLoad', planetId, transportId, unitIds })} onEvacuate={(planetId, transportIds) => issue({ type: 'battleEvacuate', planetId, transportIds })} onExit={() => setBattleId(undefined)} /></>;
  const groundDeployment = view === 'galaxy' && tab === 'surface' && planet.intelStatus !== 'unscouted'
    ? groundDeploymentForPlanet(presentedState, planet.id)
    : undefined;
