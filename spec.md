...
5. Every planet is surrounded by a 600-pixel-radius gravity well on a 12800 × 8800 tactical canvas. The well is click-through for maneuver orders, while the planet center remains a docking target. Ships have persistent orbit positions and can be drag-box selected, Shift-selected, and moved as formations within the well. Valid live coordinates are never rewritten by overlap detection: dense formations may temporarily overlap or cross paths without any ship being teleported into an automatic orbit slot near the planet. Selecting one or more ships opens a compact fleet HUD with one ship glyph per unit and separate, unlabeled hull and shield bars directly beneath every glyph. Left-click is reserved for selection and inspection; right-clicking an open point inside the fleet's current gravity well issues a maneuver order. Clicking empty space outside the selected fleet's current well clears ship selection. Orbital movement and landing approaches are deliberately slow and occur visibly over time rather than teleporting to the destination.
